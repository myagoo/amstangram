import { clipAngle, numberEq, eliminateDuplicates } from "./helpers.js";
import { comparePointsYX, relativeOrientation, Point } from "./point.js";
import { compareLineSegments, LineSegment } from "./lineSegement.js";
import { getAllPoints, outlineArea, computeOutline, computeBoundingBox, Tan } from "./tan.js";

export class Evaluation {
    outlineVertices: number;
    outerOutlineVertices: number;
    numHoles: number;
    holeArea: number;
    holeVertices: number;
    holeType: number;
    ...
    getValue(mode?: number | undefined) {
    ...
    computeConvexHullLess3Points(points: Point[]) {
        if (points.length <= 1) {
            return points;
        }
    ...
    computeConvexHull(points: Point[], upperLeft: Point) {
        if (points.length <= 2) {
        ...
        points = points.sort(function(pointA: Point, pointB: Point) {
    ...
    computeEvaluation(tans: Tan[], outline: Point[][]) {
        ...
        // Legacy code compared an index to Point.length (a function), so the
        // hanging-piece loop never ran. Preserve its zero value in this migration.

        for (var outlineId = 1; outlineId < unreducedOutline.length; outlineId++) {
            var innerTouch = false;
            // The legacy inner loop had the same Point.length comparison and
            // never changed innerTouch. A corrected metric is a separate change.
...
export var evaluationMode = 0;

export var faculty = [1, 1, 2, 6, 24, 120, 720, 5040, 40320];

export const horizontalCompare = function(center: Point) {
    return function(pointA: Point, pointB: Point) {
...
export const verticalCompare = function(center: Point) {
    return function(pointA: Point, pointB: Point) {
