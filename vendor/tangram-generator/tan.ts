import { generating, eliminateDuplicates } from "./helpers.js";
import { IntAdjoinSqrt2 } from "./intadjoinsqrt2.js";
import { comparePoints, relativeOrientation, bothPointsMultipleTimes, Point } from "./point.js";
import { compareLineSegments, LineSegment } from "./lineSegement.js";
import { Directions, InsideDirections } from "./directions.js";

export class Tan {
    tanType: number; anchor: Point; orientation: number; points?: Point[]; segments?: LineSegment[]; insidePoints?: Point[];
    constructor(tanType: number, anchor: Point, orientation: number) {
...
export var areaSum = new IntAdjoinSqrt2(576, 0);

export const getAllPoints = function(tans: Tan[]) {
    var points: Point[] = [];
...
export const outlineArea = function(outline: Point[]) {
...
export const tanSumArea = function(tans: Tan[]) {
    var area = 0;
...
export const outlineContainsAll = function(outline: Point[], allPoints: Point[]) {
...
export const computeSegments = function(allPoints: Point[], tans: Tan[]) {
...
export const findMinSegments = function(lastSegment: LineSegment, segments: LineSegment[]) {
    var minAngle = 360;
    var minIndex = -1;
...
export const findMaxSegments = function(lastSegment: LineSegment, segments: LineSegment[]) {
    var maxAngle = 0;
    var maxIndex = -1;
    for (var segmentId = 0; segmentId < segments.length; segmentId++) {
        var currentAngle = segments[segmentId].angleTo(lastSegment)!;
...
export const computeOutlinePart = function(allPoints: Point[], allSegments: LineSegment[], angleFinder: typeof findMaxSegments, hole: boolean, reduce: boolean | typeof findMinSegments) {
    ...
    outline.pop();
    return [outline, allSegments] as [Point[], LineSegment[]];
};

export const computeHole = function(allPoints: Point[], allSegments: LineSegment[], reduce: boolean | typeof findMinSegments) {
    if (allPoints.length === 0 || allSegments.length === 0) {
        return;
    }
...
export const computeOutline = function(tans: Tan[], reduce: boolean) {
    /* First calculate all line segments involved in the tangram. These line
     * segments are the segments of each individual tan however split up at points
     * from other tans */
    var outline = [];
...
export const computeBoundingBox = function(tans: Tan[], outlines?: Point[][]) {
    const outline = outlines === undefined ? getAllPoints(tans) : outlines[0];
...
export const containsPoint = function(outline: Point[], point: Point) {
