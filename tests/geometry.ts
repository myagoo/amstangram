...
export function readTan(id: string) {
  const pieces = paper.project.activeLayer.children.find(
    (item) =>
      ...
      item.children.some((child) => child.data.id === id)
  ) as PiecesGroup
...
export function readScene() {
  const particles = paper.project
    .getItems({ class: paper.Path })
    .filter((item) => typeof item.data.index === "number")
  return {
    ...
    particles: particles.map((item) => ({
      ...
      color: item.fillColor?.toCSS(true),
    })),
  }
}
...
export function checkGeometry() {
  const pieces = paper.project.activeLayer.children.find(
    (item) =>
      item instanceof paper.Group &&
      item.children.length === 7 &&
      item.children[0].data.id === "st1"
  ) as PiecesGroup
  const originalArea = pieces.children.reduce(
    (sum, piece) => sum + Math.abs(piece.children.display.area),
    0
  )
  ...
  pieces.children.forEach((piece, index) => {
    piece.position = new paper.Point(index * 2000, 200)
  })
  ...
  pieces.children[1].translate(
    first.children.display.segments[1].point.subtract(
      pieces.children[1].children.display.segments[0].point
    )
  )
  pieces.children.forEach((piece) => updateColisionState(piece, pieces))
  const target = pieces.children
    .slice(1)
    .reduce<Outline>(
      (outline, piece) =>
  ...
  target.remove()
  const exported = getPathData(
    pieces,
    Math.sqrt(originalArea / (16 * SMALL_TRIANGLE_BASE ** 2))
  )
  const snapTarget = new paper.Path({
    segments: [
      [0, 0],
      [100, 0],
      [0, 100],
  ...
  return {
    ...
    path: exported.path,
    complete,
    difficulties: [5, 8, 9, 16, 17, 22].map((edges) =>
      getTangramDifficulty({ edges })
    ),
  }
}
...
export function sameOutline(a: string, b: string) {
  ...
  const matches =
    ...
    firstPoints.every(
      (point, index) => point.getDistance(secondPoints[index]) < 0.0001
    )
  first.remove()
  second.remove()
  return matches
