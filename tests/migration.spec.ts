...
test("authentication failures retain their field message", async ({ page }) => {
  ...
  await page
    .locator('form[name="signin"] input[name="email"]')
    .fill("tester@example.test")
  await page
    .locator('form[name="signin"] input[name="password"]')
...
test("a seed reproduces playlist shuffling and random emojis", async ({
  page,
}) => {
...
test("a seed reproduces particle geometry and colors without changing the tan layout", async ({
  page,
}) => {
  ...
  const scene = async (seed: number, particles: boolean) => {
    await page.goto(`/?seed=${seed}`)
    await page.evaluate(
      (enabled) => localStorage.setItem("showParticles", String(enabled)),
      particles
    )
    await page.reload()
    await expect(page.locator("canvas")).toBeVisible({ timeout: 15000 })
...
test("mouse clicks rotate and flip the parallelogram; dragging moves it", async ({
  page,
}) => {
  const errors: string[] = []
  page.on("pageerror", (error) => errors.push(error.message))
  ...
  for (let click = 0; click < 4; click++) {
    await page.mouse.click(bounds.x + current.x, bounds.y + current.y, {
      delay: 20,
    })
  ...
  await page.mouse.down()
  await page.mouse.move(bounds.x + current.x + dx, bounds.y + current.y + dy, {
    steps: 12,
  })
  await page.mouse.up()
  const dragged = await read()
  expect(
    Math.hypot(dragged.x - current.x, dragged.y - current.y)
  ).toBeGreaterThan(50)
...
test("gallery play, tan transforms, and saving the current arrangement", async ({
  ...
  expect(
    await page.evaluate(
      ...
      { expected: geometry.path, actual: previewPath }
    )
  ).toBe(true)
