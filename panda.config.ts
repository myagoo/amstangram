...
const pixels = (values: Record<string, number> | number[]) =>
  Object.fromEntries(
    Object.entries(values).map(([key, value]) => [key, { value: `${value}px` }])
  )

const colorToken = (light: string, dark: string) => ({
  value: { base: light, _dark: dark },
})
const colors = {
  ...Object.fromEntries(
    Object.entries(THEMES.light.colors).flatMap(([name, light]) =>
      typeof light === "string"
        ? [
            [
              name,
              colorToken(
                light,
                String(
                  THEMES.dark.colors[name as keyof typeof THEMES.dark.colors]
                )
              ),
            ],
          ]
  ...
  pieces: Object.fromEntries(
    Object.entries(THEMES.light.colors.pieces).map(([name, light]) => [
      name,
      colorToken(
        light,
        THEMES.dark.colors.pieces[
          name as keyof typeof THEMES.dark.colors.pieces
        ]
      ),
    ])
  ),
  difficulties: Object.fromEntries(
    THEMES.light.colors.difficulties.map((light, index) => [
      index,
      colorToken(light, THEMES.dark.colors.difficulties[index]),
    ])
  ),
}

const gradient = (property: string) =>
  Object.fromEntries(
    ["lt2", "rh", "st2", "mt1", "st1", "lt1", "sq", "lt2"].map(
      (piece, index) => [
...
export default defineConfig({
  ...
  theme: {
    breakpoints: {
      sm: baseTheme.breakpoints.s,
      md: baseTheme.breakpoints.m,
      lg: baseTheme.breakpoints.l,
    },
    tokens: {
      durations: {
