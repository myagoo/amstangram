/** Class for a Tan */
var areaSum = new IntAdjoinSqrt2(576 * FACTOR, 0);
...
var areas = [96, 48, 24, 48, 48, 48].map(area => area * FACTOR);
...
var computeOutline = function (tans, reduce) {
    ...
    console.log('console ' + JSON.stringify(area))



    /* Compute possible holes */
    while ((!area.eq(areaSum) && area.toFloat() > (40001)) || !outlineContainsAll(outline[0], allPoints)) {
        outlineId++;
...
var computeBoundingBox = function (tans, outline) {
    ...
    var minX = new IntAdjoinSqrt2(100 * FACTOR, 0);
    var minY = new IntAdjoinSqrt2(100 * FACTOR, 0);
    var maxX = new IntAdjoinSqrt2(-100 * FACTOR, 0);
    var maxY = new IntAdjoinSqrt2(-100 * FACTOR, 0);
